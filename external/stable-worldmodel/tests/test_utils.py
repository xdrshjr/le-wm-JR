...
def test_pretraining_success(monkeypatch):
    ...
    pretraining(
        'fake_script.py', 'test_dataset', 'test_model', args='--epochs 10'
    )
...
def test_pretraining_parses_single_arg(monkeypatch):
    ...
    pretraining(
        'fake_script.py', 'test_dataset', 'test_model', args='batch-size=32'
    )
    ...
    def load_episode(self, ep_idx):
        return {
            'pixels': torch.randint(
                0,
                255,
                (self.num_steps, 3, self.height, self.width),
                dtype=torch.uint8,
            ),
            'pixels_alt': torch.randint(
                0,
                255,
                (self.num_steps, 3, self.height, self.width),
                dtype=torch.uint8,
...
def test_record_video_max_steps(tmp_path):
    ...
    record_video_from_dataset(
        tmp_path, dataset, episode_idx=0, max_steps=10, fps=10
    )
...
def test_record_video_invalid_view_raises(tmp_path):
    ...
    with pytest.raises(AssertionError, match='not in dataset key names'):
        record_video_from_dataset(
            tmp_path, dataset, episode_idx=0, viewname='nonexistent'
        )
