...
swm_env = [
    env_id for env_id in gym.envs.registry.keys() if env_id.startswith('swm/')
]
...
def test_each_env(env, temp_path):
    ...
    world.record_dataset(
        f'{ds_name}-2', episodes=1, seed=2347, cache_dir=temp_path
    )
    ...
    world.record_video_from_dataset(
        f'{temp_path}/{ds_name}', ds_name, episode_idx=0, cache_dir=temp_path
    )
    world.record_video_from_dataset(
        f'{temp_path}/{ds_name}-2', ds_name, episode_idx=0, cache_dir=temp_path
    )
    ...
    dataset_video = iio.imread(
        f'{temp_path}/{ds_name}/episode_0.mp4', index=None
    )

    recorded_video2 = iio.imread(
        f'{temp_path}/{ds_name}-2/env_0.mp4', index=None
    )
    dataset_video2 = iio.imread(
        f'{temp_path}/{ds_name}-2/episode_0.mp4', index=None
    )
    ...
    assert (
        recorded_video.shape == dataset_video.shape
    )  # both videos should have the same shape
    assert (
        recorded_video2.shape == dataset_video2.shape
    )  # both videos should have the same shape
    assert (
        recorded_video2.shape == recorded_video.shape
    )  # both videos should have the same shape
    ...
    assert len(dataset) == EPISODE_LENGTH, (
        'Dataset length should be equal to episode length'
    )
    assert len(dataset2) == EPISODE_LENGTH, (
        'Dataset lengths should be identical'
    )
    assert set(dataset.column_names) == set(dataset2.column_names), (
        'Dataset columns should be identical'
    )
    ...
    for key in world.infos:
        if key.startswith('pixels'):
            assert world.infos[key].shape[:4] == (1, 1, 224, 224), (
                f'image shape is {world.infos[key].shape}'
            )
...
def test_ep_idx_in_recorded_dataset(temp_path):
    ...
    world.set_policy(policy)
    world.record_dataset(
        ds_name, episodes=NUM_EPISODES, seed=123, cache_dir=temp_path
    )
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        assert len(ep_idx) == total_steps, (
            f'ep_idx length {len(ep_idx)} should equal total steps {total_steps}'
        )
