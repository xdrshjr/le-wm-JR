...
class RocketLandingEnv(RocketBaseEnv):
                ...
                'rocket': swm.spaces.Dict(
                    {
                        'body_color': swm.spaces.RGBBox(
                            init_value=np.array(
                                [255, 204, 0], dtype=np.uint8
                            )  # yellow
                        ),
                        'fin_color': swm.spaces.RGBBox(
                            init_value=np.array(
                                [51, 51, 51], dtype=np.uint8
                            )  # grey
                        ),
    ...
    def _modify_rocket_urdf(self) -> str:
        ...
        pyflyt_dir = os.path.dirname(os.path.realpath(PyFlyt.__file__))
        original_urdf = os.path.join(
            pyflyt_dir, 'models/vehicles/rocket/rocket.urdf'
        )
        ...
        for material in root.findall(".//material[@name='yellow']"):
            ...
            color.set(
                'rgba',
                f'{body_color[0]:.3f} {body_color[1]:.3f} {body_color[2]:.3f} 1.0',
            )
        ...
        for material in root.findall(".//material[@name='grey']"):
            ...
            color.set(
                'rgba',
                f'{fin_color[0]:.3f} {fin_color[1]:.3f} {fin_color[2]:.3f} 1.0',
            )
        ...
        for material in root.findall(".//material[@name='black']"):
            ...
            color.set(
                'rgba',
                f'{leg_color[0]:.3f} {leg_color[1]:.3f} {leg_color[2]:.3f} 1.0',
            )
    ...
    def _modify_pad_urdf(self) -> str:
        ...
        if material is None:
            ...
            color_elem.set(
                'rgba',
                f'{pad_color[0]:.3f} {pad_color[1]:.3f} {pad_color[2]:.3f} 1.0',
            )
    ...
    def reset(
        self, *, seed: None | int = None, options: None | dict = None
        ...
        variation_controls_offset = any(
            opt == 'all'
            or opt == 'environment'
            or opt.startswith('environment.start_horizontal_offset')
            for opt in variation_options
        )

        # Apply start position variations
        start_height_ratio = self.variation_space['environment'][
            'start_height_ratio'
        ].value
        start_offset = self.variation_space['environment'][
        ...
        self.start_pos = np.array(
            [
                [
                    start_offset[0],
                    start_offset[1],
                    self.ceiling * start_height_ratio,
                ]
            ],
            dtype=np.float64,
        )
        self.start_orn = np.array(
            [[start_tilt[0], start_tilt[1], 0.0]], dtype=np.float64
        )
        ...
        base_pos, base_orn = p.getBasePositionAndOrientation(
            rocket_id, physicsClientId=self.env._client
        )
        ...
        if options['accelerate_drop']:
            ...
            self.env.resetBaseVelocity(
                rocket_id, lin_vel.tolist(), ang_vel.tolist()
            )
        ...
        if variation_options and (
            ...
            rocket_id = self.env.drones[0].Id
            body_color = (
                self.variation_space['rocket']['body_color'].value / 255.0
            )
            fin_color = (
                self.variation_space['rocket']['fin_color'].value / 255.0
            )
            leg_color = (
                self.variation_space['rocket']['leg_color'].value / 255.0
            )
            booster_color = (
                self.variation_space['rocket']['booster_color'].value / 255.0
            )
            ...
            for i in range(-1, num_joints):
                if i == -1:
                    ...
                    p.changeVisualShape(
                        rocket_id,
                        i,
                        rgbaColor=list(body_color) + [1.0],
                        physicsClientId=self.env._client,
                    )
                else:
                    joint_info = p.getJointInfo(
                        rocket_id, i, physicsClientId=self.env._client
                    )
                    ...
                    if 'fin' in link_name.lower():
                        ...
                        p.changeVisualShape(
                            rocket_id,
                            i,
                            rgbaColor=list(booster_color) + [1.0],
                            physicsClientId=self.env._client,
                        )
        ...
        if self.render_mode is not None:
            ...
            landed_orientation = p.getQuaternionFromEuler(
                [0.0, 0.0, 0.0]
            )  # upright
            p.resetBasePositionAndOrientation(
            ...
            p.restoreState(
                stateId=init_state_id, physicsClientId=self.env._client
            )
            p.removeState(
                stateUniqueId=init_state_id, physicsClientId=self.env._client
            )
        else:
    ...
    def compute_state(self) -> None:
        ...
        rotation = np.array(p.getMatrixFromQuaternion(quaternion)).reshape(
            3, 3
        )
    ...
    def compute_term_trunc_reward(self) -> None:
        ...
        if not self.sparse_reward:
            ...
            deceleration_progress = (
                (
                    self.ground_lin_vel[-1]
                    - self.previous_ground_lin_vel[-1]
                    + 1.0
                )
            ...
            self.reward += (
                ...
                )  # reward for staying over landing pad
                + (
                    10.0 * lateral_progress
                )  # reward for making progress to landing pad
        ...
        if (
            np.linalg.norm(self.previous_ang_vel) > 10.0
            or np.linalg.norm(self.previous_lin_vel) > 5.0
        ):
