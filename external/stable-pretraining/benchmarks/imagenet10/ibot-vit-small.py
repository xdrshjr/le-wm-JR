"""iBOT ViT-S/16 on ImageNet-10 (Imagenette). 20 epochs, 1 GPU, no W&B.

Note: iBOT only uses global views (no multi-crop), so we instantiate the
multi-crop dataloader with ``n_local=0``.
"""
...
from stable_pretraining.methods.ibot import iBOT
